...
# conversation_id → {merchant_id, customer_id, turns, trigger_id, sent_bodies}
conversations: dict[str, dict] = {}

# suppression keys already sent
...
def detect_intent(message: str) -> str:
    """Pre-screen merchant intent before calling LLM."""
    low = message.lower().strip()
    join_words = ["join", "judrna", "jodna", "sign up", "subscribe", "enroll", "register"]
    positive = ["yes", "haan", "ok", "okay", "go ahead", "send it", "send me",
...
3. No preamble ("I hope you're doing well…"). Get straight to the point.
...
5. Use service+price offers ("Haircut @ ₹99") NOT generic discounts ("flat 30% off").
6. Dentists/pharmacies: NEVER use "cure", "guaranteed", "best in city". Clinical peer tone only.
7. Honor language preference: hi-en mix→mix Hindi+English naturally. hi→Hindi. english→English only.
...
11. NEVER invent data. Only cite sources explicitly in the digest.
12. urgency 4-5 → convey urgency without being alarmist.

AVOID: multiple CTAs, "AMAZING DEAL!", re-introducing yourself, buried CTA, generic offers, long preambles, repeating prior messages.
...
RULES:
1. Merchant said yes/accept → deliver what was promised, advance the action.
2. Merchant said "join" / wants to sign up → immediately action: "Great! Takes 2 min — reply YES to confirm and I'll send the onboarding link."
3. Merchant asked a question → answer concisely using context data only.
4. Merchant said no/not interested → action=end, graceful exit.
5. Auto-reply detected → action=wait 1800 seconds.
6. Keep replies under 120 words. Match language pref. Single CTA at end.
7. Never repeat a previous message verbatim.

OUTPUT: Valid JSON only. No markdown.
{
  "action": "send",
  "body": "<message or null>",
  "cta": "open_ended",
  "wait_seconds": null,
  "rationale": "<1-2 sentences>"
}"""
...
def compose_message(
    ...
    """Use Gemini to compose a proactive message."""
    cat_slim = {
    ...
    try:
        raw = call_gemini(COMPOSE_SYSTEM, user_prompt, max_tokens=512)
...
def compose_reply(
    ...
    """Use Gemini to compose a reply."""
    # Fast-path: no LLM needed for clear rejections
    intent = detect_intent(merchant_message)
    if detect_auto_reply(merchant_message):
        return {"action": "wait", "body": None, "cta": "open_ended",
                "wait_seconds": 1800, "rationale": "Auto-reply detected; backing off 30 min"}
    if intent == "reject":
        return {"action": "end", "body": None, "cta": "open_ended",
                "wait_seconds": None, "rationale": "Merchant not interested; graceful exit"}
    ...
    try:
        ...
        return {"action": "send", "body": "Got it! Let me check and get back to you shortly.",
                "cta": "open_ended", "wait_seconds": None, "rationale": "Fallback reply"}
...
async def metadata():
    return {
        ...
        "approach": (
            ...
            "Gemini 1.5 Flash composes every message with explicit compulsion-lever selection. "
            "Auto-reply detection, intent routing, suppression dedup, anti-repetition."
        ),
...
async def tick(body: TickBody):
    ...
    for trg_id, trg in trigger_payloads:
        ...
        if not merchant_id or merchant_id in messaged_merchants:
            continue
...
async def reply(body: ReplyBody):
    ...
    if action == "send":
        ...
        if reply_body in conv["sent_bodies"]:
            reply_body = reply_body + " — shall I set that up now?"
